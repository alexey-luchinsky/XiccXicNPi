...
EvtComplex EvtWHad::BWKK(double s, int i) {
    double m2 = pow(mRho[i], 2);
    EvtComplex qs = pcm(s);
    EvtComplex qm = pcm(m2);
    EvtComplex rat = qs / qm;
    EvtComplex rat3 = rat * rat*rat;
    EvtComplex gamma = m2 / s * rat3 * gamma0[i];
    EvtComplex I(0, 1);
    EvtComplex denBW = m2 - s - I * sqrt(s) * gamma;
    if( abs(denBW)<1e-10 ) return 0;
    return cK[i] * m2 / denBW;
}

EvtVector4C EvtWHad::WCurrent_KSK(EvtVector4R pKS, EvtVector4R pKplus) {
    double s = (pKS+pKplus).mass2();
    EvtComplex f = BWKK(s, 0) + BWKK(s, 1) + BWKK(s,2);
    return f*(pKS-pKplus);
}

EvtComplex EvtWHad::pcm(double s) {
    if(abs(s)<1e-10) return 0;
    const double mpi2 = pow(0.140, 2);
    double pcm2 = 1 - 4 * mpi2 / s;
    if (pcm2 >= 0) return EvtComplex(sqrt(pcm2), 0);
    else return EvtComplex(0, sqrt(-pcm2));
}
...
EvtWHad::EvtWHad() {
    mRho[0] = 0.77511;
    gamma0[0] = 0.1491; // rho(770)
    mRho[1] = 1.465;
    gamma0[1] = 0.400; // rho(1450)
    mRho[2] = 1.72;
    gamma0[2] = 0.250; // rho(1700)
    mRho[3] = 2.150;
    gamma0[3] = 0.310; // rho(2150), PR D76 092005
